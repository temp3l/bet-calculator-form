 ...
 */
const skewedDistribution = (Props: Props, START_CFG: number, END_CFG: number, mean: number, isUniform: boolean = false): ordersProps => {
  ...
  for (let i = 0; i < n_tp; i++) {
    ...
    orders.orders.push({
      ...
      price: parseFloat((start_ + i * increment).toFixed(decimal_rounding[symbol])),
      ordType: 'Limit',
  ...
  const total_quantity = orders.orders.reduce((total: number, n: any): number => total + n.orderQty, 0);
  // Quantity always stays the same
  if (total_quantity < quantity) {
    orders.orders[orders.orders.length - 1].orderQty = orders.orders[orders.orders.length - 1].orderQty + quantity - total_quantity;
  }
  return orders;
};
